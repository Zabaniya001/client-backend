use crate::player_records::PlayerRecords;
use crate::steamapi::{self, steam_api_loop};
use crate::{gamefinder, web};
...
use crate::demo::demo_loop;
use crate::io::{Commands, IOManager};
use crate::launchoptions::LaunchOptions;
use crate::settings::{ConfigFilesError, Settings};
use crate::state::SharedState;
use clap::{ArgAction, Parser};
use tappet::SteamAPI;

/// Usually construct with [Args::parse()]
#[derive(Parser, Debug)]
...
/// Starts the client backend running. This will create a new thread and spawn a [tokio::Runtime] in it
/// to handle all of the client operations.
pub fn start_backend(args: Args) -> (SharedState, Receiver<()>) {
    // Load settings
    ...
    // Start the async part of the program running on another thread
    std::thread::spawn(move || {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .block_on(async {
                ...
                // Steam API loop
                let (steam_api_requester, steam_api_receiver) =
        ...
        // Signals that the backend has terminated, but actually this will probably never run.
        tx.send(()).ok();
    });
